...
bash "install nodejs from source" do
  ...
  code <<-EOH
    ...
    make && \
    make install
    checkinstall -y -D --pkgname=nodejs --pkgversion=#{node[:nodejs][:version]} --pkgrelease=1 --maintainer=daniel.huesch@scalarium.com --pkggroup=ruby --pkglicense='Ruby License' make install
  EOH
